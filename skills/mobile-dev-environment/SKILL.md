...
3. **Check platform-specific tools:**
   ...
   **macOS (iOS + Android):**
   ...
   java -version            # JDK 17 (required for Android)
   ```
...
4. **Report results.** Present a checklist:
   ...
   | Expo CLI | Available | 0.18.0 | - |
   | Watchman | Missing | - | `brew install watchman` |
   | Xcode | Installed | 15.4 | - |
...
5. **Provide install commands for missing tools:**
   ...
   **JDK 17 (all platforms):**
   ```bash
   # macOS
   brew install --cask zulu@17

   # Windows
   # Download Azul Zulu JDK 17 from https://www.azul.com/downloads/
...
2. Reports: Node 20.11.0 installed, Watchman missing, Xcode 15.4 installed, Android Studio missing
...
1. **Wrong Node version** - Expo requires Node 18+. If you have an older version, use `nvm` or `fnm` to manage versions.
...
4. **JDK version mismatch** - Android Gradle builds require JDK 17. JDK 21 may cause issues with older Gradle versions.
